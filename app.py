# app.py — Output Bundle Viewer v1 (Extended)
...
st.set_page_config(page_title="Output Bundle Viewer (v1 Extended)", page_icon="📦", layout="wide")
...
CACHE_DIR = os.path.join(BASE_DIR, "CACHE"); LOGS_DIR = os.path.join(BASE_DIR, "LOGS"); EXPORTS_DIR = os.path.join(BASE_DIR, "EXPORTS")
...
with st.sidebar:
    st.title("📦 Output Bundle Viewer v1")
    st.caption("Extended: treemap, network, export, caching")
    ...
    st.caption("API keys in `.streamlit/secrets.toml`.")
...
def write_log(payload: dict):
    ...
    try:
        ...
        with open(os.path.join(LOGS_DIR, f"run_{ts}.json"), "w", encoding="utf-8") as f: json.dump(payload, f, indent=2)
    except Exception: pass
...
def cache_save(filename: str, content: bytes):
    try:
        ...
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        path = os.path.join(CACHE_DIR, f"{ts}__{normalize_name(filename)}")
...
if csv_files:
    ...
    with st.expander("Column Summary"): st.dataframe(pd.DataFrame([{"column":c,"dtype":str(df[c].dtype),"non_null":int(df[c].notna().sum()),"unique":int(df[c].nunique(dropna=True)),"sample":df[c].dropna().head(3).tolist()} for c in df.columns]), use_container_width=True)
...
def build_dataset_payload(name: str) -> dict:
    if name.lower().endswith(".csv"):
        ...
        return {"type":"json","name":name,"data":try_json_obj(name)}
    return {"type":"binary","name":name,"size":len(raw_files[name])}

if st.button("Export selected dataset"):
    if dataset_choice == "None": st.warning("Select a CSV or JSON first.")
    else:
        payload = build_dataset_payload(dataset_choice)
        ok=False; msg=""
        if export_target == "Flourish": ok,msg = export_to_flourish(payload)
        elif export_target == "ObservableHQ": ok,msg = export_to_observable(payload)
        else: msg="Export target is None."
        if ok:
            st.success(f"Exported successfully: {msg}")
            try:
                os.makedirs(EXPORTS_DIR, exist_ok=True)
                with open(os.path.join(EXPORTS_DIR, f"export_{int(time.time())}.json"), "w", encoding="utf-8") as f:
                    json.dump({"target": export_target, "dataset": dataset_choice, "payload": payload, "result": msg}, f, indent=2)
            except Exception: pass
        else: st.error(f"Export failed: {msg}")
